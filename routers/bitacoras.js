...
router.get('/bitacoras', (req, res) => {
    ...
    const columnas = 'cod_bitacora, accion, tabla_afectada, fecha_hora';

    // Definimos el nombre de la tabla
    const tabla = 'bitacoras';
...
router.post('/bitacoras', (req, res) => {
    ...
    // Definimos las columnas de la tabla bitacoras
    const columnas = 'cod_bitacora, accion, tabla_afectada, fecha_hora';

    // Armamos los valores dinámicos para el INSERT
    // OJO:
    // - cod_bitacora es numérico (sin comillas)
    // - los demás son strings o datetime (con comillas simples)
    const datos = `${cod_bitacora}, '${accion}', '${tabla_afectada}', '${fecha_hora}'`;
    ...
    db.query('CALL PA_INSERT(?, ?, ?)', [tabla, columnas, datos], (err, results) => {
        if (err) {
            ...
            res.status(201).json({
                respuesta: 'Registro insertado correctamente en la tabla bitacoras.'
            });
        }
    });
});
...
router.put('/bitacoras', (req, res) => {
    ...
    // Si es numérico, se queda sin comillas; si es texto o fecha, se encierran en comillas simples.
    let nuevoDato;
    ...
    db.query(
        ...
        (err, results) => {
            if (err) {
                ...
                res.status(200).json({
                    respuesta: 'Registro actualizado correctamente en la tabla bitacoras.'
                });
            }
        }
    );
