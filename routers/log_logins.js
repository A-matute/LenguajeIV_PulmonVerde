...
// Importamos la conexión a la base de datos (desde models/log_logins-model.js)
const db = require('../config/db-connection');

// Definimos una ruta GET para obtener todos los registros de los log_logins.
router.get('/log_logins', (req, res) => {
    ...
    const columnas = 'cod_log, fecha_hora_ingreso, fecha_hora_salida, resultado_conexion, estado_conexion, tiempo_conexion, ip_usuario, cod_usuario';

    // Definimos el nombre de la tabla.
    const tabla = 'log_logins';
...
// Ruta POST para insertar un nuevo log_logins.
router.post('/log_logins', (req, res) => {
    // Extraer cod_log y los demás campos del body
    const {
        cod_log,
        fecha_hora_ingreso,
        fecha_hora_salida,
    ...
    console.log(req.body); // Verifica qué datos están llegando al backend
    ...
    // Armamos los valores, con cod_log sin comillas (porque es número)
    const datos = `${cod_log}, '${fecha_hora_ingreso}', '${fecha_hora_salida}', ${resultado_conexion}, '${estado_conexion}', '${tiempo_conexion}', '${ip_usuario}', ${cod_usuario}`;
...
// Definimos la ruta PUT para actualizar registro en la tabla log-logins.
router.put('/log_logins', (req, res) => {
